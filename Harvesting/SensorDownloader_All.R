...
if(dev){
  serverPath <- "http://127.0.0.1:8905/SensorAPI"
}else{
...
pingSensor <- function(rec, outDir, logName, serverPath){
...
logName <- paste0(rootDir, '/ProcessingLog_SFS.csv')
...
outDir <- paste0(rootDir, '/DataDumps/SFS_All')
  ...
  pbStep(pb, step=cntr)
      ...
      if(!file.exists(outFile)){
          ...
          while( is.null(r) && attempt <= maxRetry ) {
            ...
            Sys.sleep(1)
            try(
              r <- pingSensor(rec=rec, outDir=outDir, logName=logName, serverPath = serverPath)
            )
          }
          if(attempt==maxRetry){
